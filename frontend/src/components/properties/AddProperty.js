...
const AddProperty = () => {
  ...
  const validateForm = () => {
    ...
    // Make images optional - remove this validation
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await client.post('/properties', formDataToSend, {
        headers: {
      ...
      if (details) {
        toast.error(`${message}: ${Array.isArray(details) ? details.map(e => e.msg).join(', ') : details}`);
      } else {
        toast.error(message);
      }
    } finally {
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-3">
                Property Images * (Up to 200MB each)
              </label>
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
                ...
                <label htmlFor="image-upload" className="cursor-pointer">
                  ...
                  <p className="text-sm text-gray-500">
                    PNG, JPG, WEBP, GIF up to 200MB each. Max 5 images.
                  </p>
                </label>
              </div>
